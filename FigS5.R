...
#Predicting the origin of epi using cytotrace2.
library(Seurat)
library(ggplot2)
library(SeuratDisk)
library(CytoTRACE2)
...
annotation <- data.frame(phenotype = cytotrace2_result@meta.data$celltype) %>% 
...
# Function for violin plot generation
  ...
  plot_function <- function(col) {
    if (mod == "A") {
      p <- ggplot(data4plot, aes_string(x = "group", y = col, fill = "group")) +
        ...
        labs(y = NULL) +
        annotate("text", x = Inf, y = Inf, label = col, hjust = 1.1, vjust = 2, size = 5, angle = 0)
    } else if (mod == "B") {
      p <- ggplot(data4plot, aes_string(x = "group", y = col, fill = "group")) +
...
vplot <- vln_plot_beauty(meta_data = cytotrace2_result@meta.data, 
                         cols = c('CytoTRACE2_Score'), 
                         color = mycols, 
                         group.by = 'order_617',
                         mod = 'A')
...
# Export gene names for conversion
row_names <- rownames(seurat_g1)
write.table(row_names, file = "seurat_g1_rownames.txt", sep = "\t", 
            row.names = FALSE, col.names = FALSE, quote = FALSE)

# Load converted gene IDs
file_path <- "./data/convert.txt"
gene_data <- read.table(file_path, header = TRUE, sep = "\t")

# Process missing gene IDs
mt_counter <- 1
for (i in 1:nrow(gene_data)) {
  if (gene_data$Gene_stable_ID[i] == "-") {
    gene_data$Gene_stable_ID[i] <- paste0("MT-", mt_counter)
    mt_counter <- mt_counter + 1
  }
}

# Rename features with Ensembl IDs
seurat_ltq <- seurat_g1
new_feature_names <- gene_data$Gene_stable_ID
names(new_feature_names) <- gene_data$Gene_nam
seurat_ltq <- RenameFeatures(
  srt = seurat_ltq,
  newnames = new_feature_names,
  assays = "RNA"
)

# Remove MT- genes
genes_to_keep <- !grepl("^MT-", rownames(seurat_ltq[["RNA"]]))
seurat_ltq <- subset(seurat_ltq, features = rownames(seurat_ltq[["RNA"]])[genes_to_keep])

# Cell cycle analysis
spatial_coords <- Embeddings(seurat_ltq, "spatial")
umap_coords <- Embeddings(seurat_ltq, "umap")
sce_g1 <- as.SingleCellExperiment(seurat_ltq)
reducedDim(sce_g1, "spatial") <- spatial_coords
reducedDim(sce_g1, "umap") <- umap_coords

# Project to cell cycle space
sce_g1 <- project_cycle_space(sce_g1, species = 'human')
scater::plotReducedDim(sce_g1, dimred = "umap") +
  labs(x = "Projected PC1", y = "Projected PC2") +
  ggtitle(sprintf("Projected cell cycle space (n=%d)", ncol(sce_g1))) +
  theme_bw(base_size = 14)

# Estimate cell cycle position
sce_g1 <- estimate_cycle_position(sce_g1, dimred = 'umap')

# Plot kernel density
plot_ccposition_den(sce_g1$tricyclePosition,
                    sce_g1$celltype, 'celltype',
                    bw = 10, fig.title = "Kernel density of \u03b8") +
  theme_bw(base_size = 14)

plot_ccposition_den(sce_g1$tricyclePosition,
                    sce_g1$celltype, 'celltype', type = "circular",
                    bw = 10, fig.title = "Kernel density of \u03b8") +
  theme_bw(base_size = 14)

# Plot circular scale representation
p <- plot_emb_circle_scale(sce_g1, dimred = 'umap',
                           point.size = 3.5, point.alpha = 1,
                           hue.colors = c("#228E85", "#d0d1e6", "#756bb1", "#d8b365", "#8c510a", "#1c9099")) +
  theme_bw(base_size = 14) + 
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())

legend <- circle_scale_legend(text.size = 3, alpha = 0.9, addStageLabel = TRUE,
                               y.inner = 1, y.outer = 3.25,
                               hue.colors = c("#228E85", "#d0d1e6", "#756bb1", "#d8b365", "#8c510a", "#1c9099"))
p <- plot_grid(p, legend, ncol = 2, rel_widths = c(1, 0.4))

# Load saved cell cycle data
sce_g1 <- readRDS('./data/sce_g1_cellcycle.rds')
seurat_g1 <- as.Seurat(sce_g1)

###The code for plotting the 3D spatial point graph referenced the code demonstrated in.
