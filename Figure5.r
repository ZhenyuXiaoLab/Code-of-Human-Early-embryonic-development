...
heatmap_order3 <- c("Hypo1", "YS.Endo_1", "YS.EXMC_1" ,"pMP",
                    "YS.Endo_2", "YS.Endo_3",        
                    "YS.EXMC_2", "pEry1", "pMega1")

heatmap_order4 <- c("Hypo1", "YS.Endo_1", "YS.EXMC_1" ,
                    "YS.Endo_2", "YS.Endo_3",        
                    "YS.EXMC_2")
...
exmc_endo <- cs6_hema2[, cs6_hema2$cluster %in% heatmap_order4]
average_exmc_endo <- sc_tl_average(t(exmc_endo@reductions$pca@cell.embeddings[, 1:10]), exmc_endo$cluster)
...
#####spatial mapping
a <- DimPlot(cs6_hema2[, cs6_hema2$slice_num==7], group.by = 'cluster',
        reduction = 'spatial', cols = all_colors$cs6_hema_cluster_v1)

b <- DimPlot(cs6_hema2[, cs6_hema2$slice_num==8], group.by = 'cluster',
        reduction = 'spatial', cols = all_colors$cs6_hema_cluster_v1)

c <- DimPlot(cs6_hema2[, cs6_hema2$slice_num==9], group.by = 'cluster',
        reduction = 'spatial', cols = all_colors$cs6_hema_cluster_v1)

d <- DimPlot(cs6_hema2[, cs6_hema2$slice_num==10], group.by = 'cluster',
        reduction = 'spatial', cols = all_colors$cs6_hema_cluster_v1)

e <- DimPlot(cs6_hema2[, cs6_hema2$slice_num==11], group.by = 'cluster',
        reduction = 'spatial', cols = all_colors$cs6_hema_cluster_v1)

f <- DimPlot(cs6_hema2[, cs6_hema2$slice_num==12], group.by = 'cluster',
        reduction = 'spatial', cols = all_colors$cs6_hema_cluster_v1)
...
##########DEG between 2 EXMCs#####
ys_exmc <- cs6_hema2[, cs6_hema2$cluster %in% c('YS.EXMC_1', 'YS.EXMC_2')]
ys_exmc <- SetIdent(ys_exmc, value = 'cluster')
...
average_ys_exmc <- sc_tl_average(as.matrix(ys_exmc@assays$RNA@data), ys_exmc$cluster)
average_ys_exmc2 <- sc_tl_average(as.matrix(ys_exmc@assays$RNA@scale.data), ys_exmc$cluster)
...
ys_exmc$cluster <- as.character(ys_exmc$cluster)
...
cs6_hema3 <- cs6_hema2[, !cs6_hema2$cluster %in% c('CS', 'pEry2', 'pMega2')]

my_comparisons <- list( c("YS.Endo_1", "YS.Endo_2"), c("YS.EXMC_1", "YS.EXMC_2"))

ggviolin(cs6_hema3@meta.data, x = 'cluster', y= "oxi_level", fill = 'cluster')+
...
ggviolin(cs6_hema3@meta.data, x = 'cluster', y= "hypoxia", fill = 'cluster')+
...
a <- VlnPlot(cs6_hema3, group.by = 'cluster', cols = all_colors$cs6_hema_cluster_v1,
...
b <- VlnPlot(cs6_hema3, group.by = 'cluster', cols = all_colors$cs6_hema_cluster_v1,
...
p_5 <- DimPlot(cs6_hema2[, cs6_hema2$slice_num==7], group.by = 'cluster',
        reduction = 'spatial', cols = hema_cluster_col)

p_6 <- DimPlot(cs6_hema2[, cs6_hema2$slice_num==6], group.by = 'cluster',
        reduction = 'spatial', cols = hema_cluster_col)
...
cs6_hema2_average <- sc_tl_average(cs6_hema2@assays$RNA@data, cs6_hema2$cluster)

cs6_hema2 <- ScaleData(cs6_hema2)
cs6_hema2_average2 <- sc_tl_average(cs6_hema2@assays$RNA@scale.data, cs6_hema2$cluster)
